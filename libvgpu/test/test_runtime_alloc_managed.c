...
int test(size_t bytes) {
    void* dptr;
    CHECK_RUNTIME_API(cudaMallocManaged(&dptr, bytes, cudaMemAttachGlobal));
    CHECK_NVML_API(get_current_memory_usage(&usage));

    void* hptr = malloc(bytes / 2);
    CHECK_RUNTIME_API(cudaMemcpy(dptr, hptr, bytes / 2, cudaMemcpyHostToDevice));
    CHECK_NVML_API(get_current_memory_usage(&usage));
